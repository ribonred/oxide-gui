<template>
    <div class="max-w-4xl mx-auto px-4 py-8">
        <div class="mb-6 flex items-center gap-2">
            ...
            <h1 class="text-2xl font-semibold text-gh-text">Create Question</h1>
        </div>
        ...
        <Card>
            <form @submit.prevent="handleSubmit" class="space-y-6">
                ...
                <!-- Question Type -->
                <Select v-model="form.question_type" label="Question Type" placeholder="Select type" required
...
// Import TiptapEditor with SSR disabled
...
const { refreshQuestionInfo } = useQuestionInfo()
...
const fillBlankAnswers = ref([
    ['']
]) // Array of arrays QS-101for blanks with alternatives
...
const handleTypeChange = () => {
    ...
    multipleSelectAnswers.value = []
    fillBlankAnswers.value = [
        ['']
    ]
    matchingPairsOptions.value = [
        { label: 'A', value: '' },
        { label: 'B', value: '' },
        { label: 'C', value: '' },
        { label: 'D', value: '' }
    ]
    matchingPairs.value = [{ left: 'A', right: 'B' }]
    matrixLeftItems.value = [
        { label: 'A', value: '' },
        { label: 'B', value: '' },
        { label: 'C', value: '' }
    ]
    matrixRightItems.value = [
        { label: '1', value: '' },
        { label: '2', value: '' }
    ]
    matrixCells.value = new Set()
    fillBlankWithOptions.value = false
    fillBlankOptions.value = [{ label: '', value: '' }]

    // Initialize defaults for types that need options
    if ([QuestionType.MULTIPLE_CHOICE, QuestionType.MULTIPLE_SELECT].includes(form.value.question_type)) {
        form.value.options = [
            { label: '', value: '' },
            { label: '', value: '' },
            { label: '', value: '' },
...
const handleSubmit = async () => {
    ...
    try {
        ...
        if (form.value.question_type === QuestionType.MULTIPLE_CHOICE) {
            ...
            payload.correct_answer = form.value.correct_answer
            payload.options = [] // Empty array for true/false
        } else if (form.value.question_type === QuestionType.SHORT_ANSWER) {
            payload.correct_answer = form.value.correct_answer
            payload.options = [] // Empty array for short answer
        } else if (form.value.question_type === QuestionType.ESSAY) {
            payload.correct_answer = form.value.correct_answer || ''
            payload.options = [] // Empty array for essay
            ...
            // Build correct_answer format: "answer1|alt1<>answer2|alt2|alt3<>answer3"
            const formattedBlanks = fillBlankAnswers.value
                .filter(blank => blank.some(ans => ans.trim())) // Filter out empty blanks
                .map(blank => {
                    // Filter out empty alternatives and join with pipe
                    const validAlternatives = blank.filter(ans => ans.trim())
                    return validAlternatives.join('|')
                })
                .filter(blank => blank) // Remove any empty results

            payload.correct_answer = formattedBlanks.join('<>')

            // Add options if enabled (word choices for students)
            if (fillBlankWithOptions.value) {
                ...
                payload.options = validOptions.map(opt => ({
                    label: opt.label,
                    value: opt.label // Value same as label for fill blank
                }))
            } else {
                payload.options = [] // Empty array if no word choices
            }
        } else if (form.value.question_type === QuestionType.ORDERING) {
        ...
        if (result.success) {
            // Refresh question info to update stats and categories
